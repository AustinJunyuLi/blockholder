# Theory Paper: draft_v3.tex

This is the complete LaTeX source for the manuscript. The numerical code must match 
the equations defined here. Key sections for code alignment:
- S3.3: Noise distribution (p0, p1)
- S3.6: Bidder entry (Eq 8, logistic CDF, lambda_B, Delta_S)  
- S3.7: Terminal payoff and pricing (feed-forward, Eq 9)
- S4.6: Equilibrium prices (Eq 10)
- S5: Comparative statics calibration
- App C: Calibration table
...
Under the Standing Assumptions, by Lemma~\ref{lem:endpoints}, $\Delta^{\min}(\kappa)$ attains its minimum at the extreme high liquidity limit. Assume the deterrence effect at $\kappa \to 0$ is sufficiently strong relative to an intermediate liquidity level $\tilde{\kappa}\in(0,1)$, such that $\Delta^{\min}(\tilde{\kappa}) > \lim_{\kappa \downarrow 0} \Delta^{\min}(\kappa)$. Then there exists an interior maximizer $\kappa^{\dagger}\in(0,1)$ such that
\[
\Delta^{\min}(\kappa^{\dagger})=\max_{\kappa\in[0,1]}\Delta^{\min}(\kappa).
\]
In particular, expected minority takeover gains $\Delta^{\min}(\kappa)$ are nonmonotone in $\kappa$, peaking strictly away from the extremes.
\end{proposition}
...
Under this parameterization, Assumption~\textup{(A5)} is satisfied: the net deterrence condition $\tilde{\Delta} + \tilde{m} - m_0 > \Delta_S$ holds with the baseline values. The Hold region collapses ($k_0=k_1$), so the blockholder's strategy reduces to Exit, Quiet Voice, and Public Voice. Figure~\ref{fig:cutoff-structure} in Appendix~\ref{app:figures} depicts the resulting cutoff structure.
...
\]
While the market fully prices the expected engagement into $P(X,0)$, which deters bidder entry mathematically by reducing $p(X,0)$, the fundamental value improvement $\tilde{\Delta}$ and premium wedge $\tilde{m}-m_0$ ensure that the engagement benefit remains bounded below by $\delta \min(\tilde{m}-m_0, \tilde{\Delta})$. Under baseline calibrations, this benefit strictly exceeds the cost $C(\mu)$ at the prior mean, meaning the blockholder strictly prefers Quiet Voice to Hold for a given range of signals. Thus, the Quiet Voice region does not collapse ($\omega_Q > 0$), and $\Delta^{\textup{act}}(\kappa)$ remains strictly positive as $\lim_{\kappa \downarrow 0} \Delta^{\textup{act}}(\kappa) > 0$. Nonetheless, the heightened price fully depresses $p(X,0)$, artificially suppressing the minority gains below their optimal interior potential.
...
\begin{proof}
By Lemma~\ref{lem:endpoints}, the continuous extension of $\Delta^{\min}$ to the closed interval $[0,1]$ satisfies $\Delta^{\min}(\kappa)\to m_0\PP(\textup{bid})$ as $\kappa \uparrow 1$. Because the logistic probability formulas, Bayesian posteriors, and pricing mapping are continuous in $\kappa$, the function $\Delta^{\min}(\kappa)$ is continuous on the compact set $[0,1]$. By the Weierstrass extreme value theorem, it attains a global maximum.

By the verifiable sufficient condition, there exists $\tilde{\kappa}\in(0,1)$ such that $\Delta^{\min}(\tilde{\kappa}) > \lim_{\kappa \downarrow 0} \Delta^{\min}(\kappa)$, implying that the deterrence effect at $\kappa \to 0$ strictly suppresses minority gains relative to the interior. Coupled with the upper limit converging to the baseline, the global maximizer $\kappa^\dagger$ must lie in the interior $(0,1)$, establishing the strict nonmonotonicity of minority gains in liquidity.
\end{proof}
...
\caption{Baseline numerical equilibrium (values rounded to two decimals). Parameters: $\mu=1$, $\sigma_v=0.5$, $\sigma_\varepsilon=0.5$, $\delta=0.95$, $\Delta=0.25$, $C_0=0.12$, $\chi=0.5$, $\kappa=0.5$, $m_0=0.10$, $m_1=0.30$, $\bar{S}=1.44$, $\Delta_S=0.35$, $K=0.15$, $s_\xi=0.15$, $\lambda_B=0.05$, $\rho=0.9$. Cutoffs: $k_1=k_0\approx0.82$ and $k_D\approx2.26$.}
\label{tab:example}
\end{table}
