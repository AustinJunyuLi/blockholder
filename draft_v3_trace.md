...
| T1 "How the proof goes" route map (decomposition of one derivative; Ω constancy derived; reallocation term dropped because its coefficient is zero; Part (A) by absolute values, aggregate clause by finite differences; Part (B) W_τ C_τ with both factors in [0,1] and no dominance compared; Part (C) W_T ≤ 1 with three reasons blocking a window analogue; exact equivalence and the local criterion under (T-14); no window sign) | `draft_v3_onlineappendix.tex` T1 proof, lines 2656–3052 (preamble + Steps 1–17); one sentence per step, reviewed sentence by sentence against the proof on 2026-09-01 |
...
| C1 "How the proof goes" route map (four conventions; geometric-series inverse bounded by 1/(1−L_R); contraction + IFT branch consuming (C-3); first and cross derivatives of the branch; five-term chain-rule identity with the reallocation term reappearing; remainder bounded by B^GE_r; sign constancy on a neighbourhood from (C-5); −g^PE_r and strict dominance with slack η_r; remainder's size never its sign; no region exhibited) | `draft_v3_onlineappendix.tex` C1 proof, lines 3074–3347 (conventions + Steps 1–9); reviewed sentence by sentence against the proof on 2026-09-01 |
