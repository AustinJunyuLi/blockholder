...
\title{\textbf{Exit, Voice, and Disclosure:}\\[0.3em]
\large Liquidity and Takeover Premia under Stake-Triggered Ownership Rules}
...
A rapidly maturing structural literature quantifies activism along complementary margins, and the present paper's positioning is sharpest against it. \citet{Gantchev2013} estimates the sequential costs of campaigns; \citet{AlbuquerqueFosSchroth2022} structurally estimate the binary 13D-versus-13G choice jointly with announcement returns, decomposing activist returns into treatment, stock-picking, and selection --- direct evidence that prices \emph{capitalize} activism at disclosure, the denominator channel of Proposition~\ref{prop:d7-afs}; \citet{JohnsonSwem2021} estimate a dynamic reputation model of activism whose single state variable is the activist's reputation, the accumulation stage collapsed into a lognormal campaign-cost draw standing in for price impact and trading costs; and \citet{CelentanoLevine2025} estimate an equilibrium model of activism, takeovers, and managerial discipline --- finding that activism \emph{lowers} bid premia by 13.7\% relative to a no-activist counterfactual through board bargaining, a finding Proposition~\ref{prop:d7-afs} rationalizes through the price channel without reversing the engagement wedge, with cross-sectional content ($q,\gamma,\psi$) that a presence-only channel does not carry. None of these models the trading-and-disclosure environment itself --- liquidity, order-flow inference, and the stake-triggered disclosure rule are this paper's state variables, and the premium those papers treat as an estimated constant is here an equilibrium object of tender mechanics (Appendix~\ref{app:d7}) whose sign and size vary in observable primitives. \citet{CollinDufresneFos2015} provide the reduced-form complement, showing measured price impact falls exactly when informed blockholders trade against liquidity.\footnote{\citet{BackEtAl2018} embed an activist in a continuous-time Kyle model to study liquidity and efficiency, but they do not incorporate the feedback loop between order-flow inference and bidder entry conditional on disclosure. \citet{CetemenCisternasKolbViswanathan2026} study a dynamic complement in which sophisticated activists time their trades sequentially and use market signals to coordinate, generating leader-follower dynamics among blockholders; this paper instead focuses on a single blockholder's static choice among exit, quiet voice, and threshold-triggering public voice, and how disclosure partitions order-flow inference that feeds bidder entry.}
...
Appendix~\ref{app:d5} makes the role of Assumption~\textup{(A2)} exact. At fixed prices the gross (pre-cost) Quiet-over-Hold value gap is \emph{signal-free} --- Hold and Quiet generate the same order flow and disclosure flag, so their payoffs differ only through the engagement lottery, whose value depends on the cell objects, not on $s$ (eq.~\eqref{eq:UH-UQ}); the gross slope is $\Lambda(s)\equiv0$. The Hold/Quiet margin therefore single-crosses precisely in the \emph{persuasion regime} $C'(s)<\Lambda(s)\equiv0$ --- that is, exactly under \textup{(A2)} (Proposition~\ref{prop:d5-a2}). A flat cost ($\chi=0$) makes the Hold/Quiet gap constant in $s$, and the equilibrium sits on the two-cutoff collapse face (footnote below); the four-action architecture, existence, and comparative statics survive, but the interior Hold/Quiet boundary degenerates. The ordering can invert under an \emph{entrenchment regime} ($C'(s)\ge0$ on a nondegenerate set, e.g.\ better-informed blockholders facing harsher managerial entrenchment precisely when their case is strong), which carries the opposite, falsifiable empirical signature (Definition~\ref{def:d5-regimes}, Remark~\ref{rem:d5-mech}). The role of $\chi>0$ is thus the interior Hold/Quiet margin and the tilt of engagement toward higher-signal types. The interior peak of $\Delta^{\min}(\kappa)$ survives a flat cost, with endpoint symmetry $\Delta^{\min}(0^+)=\Delta^{\min}(1^-)$ derived from the noise law (Appendix~\ref{app:d5}).
...
\paragraph{Tender-game micro-foundation of the premium wedge.} Assumption~\textup{(A3)} is the conclusion of an explicit two-layer micro-foundation rather than a free-standing primitive. In Appendix~\ref{app:bg} I show that if the bid-conditional premia $(m_0,m_1)$ arise from generalized Nash bargaining between the target's control bloc (bargaining weight $\theta\in(0,1)$) and the bidder over the takeover surplus $\bar S+\xi$, and if engagement raises the bloc's disagreement payoff by an appropriable amount $\lambda\,\rho\,\Delta_{\mathrm{eng}}>0$ (Condition~\ref{cond:bg}), then the wedge is the closed form $m_1-m_0=(1-\theta)\lambda\rho\Delta_{\mathrm{eng}}$, invariant to the synergy draw $\xi$ (Theorem~\ref{thm:bg-A3}). In Appendix~\ref{app:d7} I then close the step earlier drafts left open: solving the complete free-rider tender game at the disagreement node (in the institution of \citet{GrossmanHart1980}, with the outside-option reading of \citet{BinmoreRubinsteinWolinsky1986}) \emph{derives} the appropriability coefficient,
\[
\lambda \;=\; 1-q\,(1-\gamma)\,\psi\;\in\;[0,1],
\]
where $q$ is the equilibrium fringe-raid probability (the symbol is local to this tender-game discussion and to Appendix~\ref{app:d7}; it is unrelated to the blockholder's trade direction $q$ of Section~\ref{sec:model}), $\gamma\in[0,1]$ the portability of the activist's improvement under an alternative acquirer, and $\psi$ a pivotality factor (Proposition~\ref{prop:d7-lambda}). Assumption~\textup{(A3)} then holds \emph{except on a characterized boundary}: it fails only when fringe raids are certain, fully supersede the activist's improvement, and cannot be blocked (Theorem~\ref{thm:d7-A3}). The same appendix shows that the \emph{measured} bid premium --- the offer over a market price that already capitalizes activism --- declines in activism evidence whenever $\lambda$ is small even though $m_1\ge m_0$ throughout (Proposition~\ref{prop:d7-afs}), reconciling the model with the negative premium effect estimated by \citet{CelentanoLevine2025}. The reduced form used in the body is the constant-$\theta$, $\lambda$-at-baseline special case.
...
\footnote{Assumption~\textup{(A5)} bounds the strength of the price-to-entry feedback loop and ensures that comparative statics are well-defined. Under the Gaussian bidding rule, a sufficient condition \emph{for the sensitivity clause} is $\delta/\sigma_\xi < 1/\phi(0)$, where $\phi(\cdot)$ is the standard normal density; the uniqueness clause is the separate inner-price contraction stated as Assumption~\textup{(A5a)} below, whose baseline margins are discussed in Remark~\ref{rem:A5margins}.}
...
\quad \eta>0,
\end{equation}
the extra slope $\eta$ on Public Voice arising from the extra share held into the terminal date and from the takeover term, which is nondecreasing in $\vhat$. Because the per-share slope in \eqref{eq:U-affine} is $\deltaliq\,h\,\E_z[1-p(X,D)]$ (slopes here are normalized by the common Hold/Quiet factor), the second share contributes $2\,\E_z[1-p(X,1)]$ against $\E_z[1-p(X,0)]$, so $\eta>0$ is \emph{conditional}: it holds whenever the disclosed bid probability satisfies $\bar p_1\equiv\E_z\,p(X,1)\le\tfrac12$ --- Lemma~\ref{lem:BP-BQ}'s condition, which holds with a wide margin at the baseline (disclosure deters bids: $\bar p_1\approx0.013$). The component $\Tmap_1$ ($\Tmap_2$, $\Tmap_3$) is the Exit/Hold (Hold/Quiet, Quiet/Public) indifference signal.
...
\label{lem:selfmap}
Under Assumptions~\textup{(A1)}, \textup{(A2)}, \textup{(A4)}, \textup{(A5a)} (so that the cell-level price is single-valued) and the slope ordering $\eta>0$ of Lemma~\ref{lem:BP-BQ}, the map $\Tmap$ is continuous and maps $\Theta$ into itself, $\Tmap(\Theta)\subseteq\Theta$, including the collapse faces.
\end{lemma}
...
Under Assumptions~\textup{(A1)}, \textup{(A2)}, \textup{(A4)}, \textup{(A5a)} and the slope ordering $\eta>0$ of Lemma~\ref{lem:BP-BQ}, an equilibrium in (weakly ordered) cutoff strategies exists.
\end{proposition}
...
\begin{proof}
The payoff slopes in $\vhat$ differ between Public and Quiet Voice through two channels. (1) \emph{Holdings:} by \eqref{eq:U-affine} the slope of action $(q,a)$ in $\vhat$ is $\deltaliq\,(1+q)\,\E_z[1-p(X,D)]$; Public Voice holds two shares into the terminal date against Quiet Voice's one, contributing $\deltaliq\bigl[2\,\E_z(1-p(X,1))-\E_z(1-p(X,0))\bigr]$, which is strictly positive whenever $\bar p_1\equiv\E_z\,p(X,1)\le\tfrac12$ (then $2(1-\bar p_1)\ge1\ge1-\bar p_Q$, strictly unless both bind). (2) \emph{Takeover:} both carry the engagement term $(\mt-m_0)\,p$; under disclosure ($D=1$ for Public Voice) the takeover probability is $p(X,1)=1-\Phi(\mathcal{T})$ with $\mathcal{T}=(\bar{m}(X,1)+\Kcost-\Sbar+P(X,1))/\sigxi$, and $\bar{m}(X,1)$ rises with the disclosed posterior, which rises with $s$. Hence $\partial p/\partial\vhat=\phi(\mathcal{T})(\partial\bar{m}/\partial\vhat)/\sigxi\ge0$, so the takeover channel adds a nonnegative amount to the Public slope. Therefore
\[
\eta=\underbrace{\deltaliq\bigl[2(1-\bar p_1)-(1-\bar p_Q)\bigr]}_{>0\text{ iff }\bar p_Q>2\bar p_1-1}
+\underbrace{(\mt-m_0)\,\frac{\phi(\mathcal{T})}{\sigxi}\,
\frac{\partial\bar{m}}{\partial\vhat}}_{\ge0}\;>\;0
\quad\text{when }\bar p_1\le\tfrac12,
\]
giving $B(P)>B(Q)$ under the displayed condition. The condition $p(X,1)\le\tfrac12\iff \mathcal{T}\ge0\iff$ \eqref{eq:BPBQ-suff} (using $\bar{m}(X,1)=\mt$ on the disclosed branch) additionally places the takeover channel on the rising part of $\Phi$, making the slope gap robust to perturbations of the disclosed price.\footnote{\eqref{eq:BPBQ-suff} is the transparent primitive bound and holds comfortably at the baseline, where the disclosed takeover probability is $p^\star=0.0131$ ($\mathcal{T}=+2.22$): disclosure \emph{deters} bids ($p(X,1)<p(X,0)$ at every reached cell; cf.\ the bid-incidence discussion in Section~\ref{sec:comp-statics}), so $2(1-\bar p_1)\approx1.97$ and the holdings channel secures $\eta>0$ with a wide margin.}
\end{proof}
...
\label{rem:A5margins}
The sufficient rule \eqref{eq:A5a} must hold cell by cell, and the binding cell at the baseline is the \emph{Exit} cell $(X,D)=(-2,0)$, where the bid probability is largest: $p^\star=0.806$, so the direct term is $\deltaliq\,p^\star\approx0.766$, and the worst-case feedback term (ii) adds $\deltaliq\,|P+\bar m-\Vhat|\,\phi(0)/\sigxi\approx0.27$, for a sum $\approx1.04>1$. So the conservative sufficient rule does \emph{not} certify $|\Psifp'|<1$ at every cell. \textbf{We therefore state Assumption~\textup{(A5a)} as a maintained assumption}, not a derived inequality. It is nonetheless mild: the rule bounds $\phi(\mathcal{T})$ by $\phi(0)$ and treats both channels as additive at their worst case; on every \emph{other} reached cell the sum is well below one (on the disclosed branch, where $p^\star=0.013$, it is $\approx0.18$); and the inner fixed point is found numerically without difficulty at every $\kp$ (residuals $\sim10^{-10}$ at fully converged points; Remark~\ref{rem:numreg}). The reader should read \eqref{eq:A5a} as the transparent primitive condition, binding only on the high-$p$ Exit cell under worst-case bounds.
\end{remark}

\begin{remark}[Uniqueness as a numerical regularity, not a proposition]
\label{rem:numreg}
We do not claim global uniqueness as a theorem. Assumption~\textup{(A6)} (contraction of $\Tmap$) would deliver it, but we cannot derive $\|D\Tmap\|_\infty<1$ from primitives without further structure on the cross-cutoff Jacobian. At the baseline calibration the equilibrium located by the solver is numerically unique: a multistart search with $30$ random ordered seeds at $\kappa\in\{0.2,0.5,0.8\}$ converges in all $30/30$ cases to a single fixed point per $\kappa$ (residuals $\sim4\times10^{-10}$), and at the hump peak $\kappa^\dagger=0.59$ the cold-start and both warm-started solves agree to $\Dmin$-spread $6.9\times10^{-9}$. We label this a \emph{numerical regularity} at the calibration, not a uniqueness theorem; the comparative statics below are shown to hold across all fixed points the multistart locates, following the methodological precedent of \citet{EdmansGoldsteinJiang2015}.
\end{remark}
...
($\E[\pi\mid D=0]$ traces $0.68\to0.58\to0.63$ at $\kappa=0.2,0.5,0.8$, and back up toward the endpoints, because
...
it is symmetric at the endpoints in the limit sense,
...
Appendix~\ref{app:d8} sharpens this division in both directions. In one
direction, Theorem~\ref{thm:d8-region} bounds the GE channel by an
\emph{inversion-free} quantity $\bar B(\kappa)$ computable from a single
equilibrium solve (the contraction modulus of Assumption~\textup{(A6)} replaces
the unknown path derivative), and certifies single-peakedness, up to
$\varepsilon$-localization of the peak, as a \emph{theorem} on the interior
region where $\bar B$ is dominated: strictly monotone on either side of a
small ball around the channel-(A) peak, global maximum interior to the ball
--- at the baseline, $\kappa\in[0.35,0.825]$ under the inversion-free bound and
$[0.30,0.85]$ under the exact implicit-function bound, comfortably containing
$\kappa^{\dagger}$ (Corollary~\ref{cor:d8-baseline}, Figure~\ref{fig:ge-decomposition}).
In the other direction, Proposition~\ref{prop:d8-counter} certifies that
Hypothesis~\ref{hyp:d1-cutoffshift} \emph{fails} at the high-$\sigma_\xi$
robustness cells: channel (A) remains single-peaked there, yet the
full-equilibrium profile is a trough, so the GE channel genuinely overturns the
shape and no calibration-free version of the hypothesis can be true. The
hump-versus-trough boundary is itself economic --- channel (A)'s amplitude
scales with the sensitivity of bidder entry to expected resistance, so GE
feedback dominates exactly where entry is premia-insensitive
(Remark~\ref{rem:d8-boundary}).
...
\]
where $\bar{p}_1 \equiv p(P^*(X,1),1)$ is the (constant) bid probability on the disclosed branch. Only the inferred component depends on $\kappa$ through the posteriors $\pi(X,0)$, and it carries the weight $\PP(D=0)=1-\omega_P$. Consequently
\[
\Bigl|\frac{\partial \Delta^{\textup{act}}}{\partial\kappa}\Bigr|
=(1-\omega_P)\,(\tilde m-m_0)\,
 \Bigl|\frac{\partial}{\partial\kappa}\,
 \E\bigl[\pi(X,0)\,p(X,0)\mid D=0\bigr]\Bigr| :
\]
the liquidity sensitivity is proportional to the undisclosed mass $1-\omega_P$. \emph{Holding the conditional sensitivity of the inferred component fixed}, shifting probability mass toward disclosed engagement (larger $\omega_P$) attenuates the liquidity sensitivity of the activism-driven premium.
\end{proposition}

\textit{Proof sketch.} In disclosed states, $\pi(X,1)=1$ and all pricing objects are $\kappa$-invariant (Appendix~\ref{app:proof-disclosed-invariance}), so the disclosed component contributes zero to $\partial\Delta^{\textup{act}}/\partial\kappa$, and the display follows by differentiating the decomposition. The qualifier is substantive: a configuration change that raises $\omega_P$ moves $k_D$, which also alters the conditional term $\E[\pi(X,0)p(X,0)\mid D=0]$; the proposition signs only the weight channel, which is the mechanical and dominant effect in the calibrated region (Figure~\ref{fig:disclosure}). \hfill$\square$
...
Figure~\ref{fig:nonmonotone} displays the headline comparative static: a nonmonotone relation between liquidity and expected minority takeover gains. As $\kappa$ rises from low levels, $\Delta^{\min}(\kappa)$ initially increases because bid incidence grows. At higher liquidity, activism-related premia contribute less, and $\Delta^{\min}(\kappa)$ declines. The vertical dashed line marks the interior turning point $\kappa^{\dagger}$.
...
This section carries the two positive results---the liquidity hump and the disclosure-attenuation mechanism---into a \emph{normative} register, asking when they translate into planner statements about total surplus and minority welfare once prices and premia are netted as pure transfers. I treat welfare as a genuine planner problem rather than as a corollary of the
...
$\Delta^{\min}(0^+)=\Delta^{\min}(1^-)$ (Lemma~\ref{lem:endpoints}; strict
positivity of the common limit is a numerical fact at the calibration,
$\approx0.067$). The interior
...
$k_D$ flips the disclosure flag. Bids arrive on \emph{both} branches of the model
(eq.~\eqref{eq:bid-prob} assigns interior bid probability to every reached cell),
but at very different rates and prices: disclosure \emph{deters} bids --- at the
baseline the disclosed bid probability is $p_1^{\ast}=0.013$ against an average
$\bar p_Q\approx0.35$ on the quiet branch --- while raising the threshold a
successful bid must clear. The marginal type therefore undergoes a discrete
switch between two different bid lotteries even though the planner's objective
varies smoothly with $k_D$.

\paragraph{The blockholder's $k_D$, derived.}
By the monotone best-response structure (Lemmas~\ref{lem:BP-BQ} and~\ref{lem:monotone}) the difference $U_P(s)-U_Q(s)$ is strictly
...
\begin{lemma}[The reclassification jump is a bid-screening term]
...
double-count the truncation terms below). Under both Quiet and Public Voice the
...
does not cancel is the \emph{bid lottery}, which differs across the two branches:
\begin{equation}
\Sigma_P(k_D)-\Sigma_Q(k_D)
 =\bigl(p_1^{\ast}-\bar p_Q\bigr)\Big(\bar S-K-\E[v\mid s{=}k_D]-\tilde\Delta\Big)
   +\sigma_\xi\Bigl(\phi(T_1)-\E_z\,\phi(T_{X,0})\Bigr),
\label{eq:reclass-real}
\end{equation}
where $p_1^{\ast}=1-\Phi(T_1)$ is the disclosed-branch bid probability with
$T_1=(\tilde m+K-\bar S+P^{\ast}(\cdot,1))/\sigma_\xi$ (eq.~\eqref{eq:bid-prob}
with $\pi=1$, $\bar m(\cdot,1)=\tilde m$), and $\bar p_Q=\E_z\,p(X,0)$,
$T_{X,0}$ are the quiet-branch analogues: a Quiet type generates $X=z$ with the
noise law, and each cell $(X,0)$ carries its own bid probability and threshold.
\end{lemma}

\begin{proof}
By \eqref{eq:welfare-net-d2} an engaging type contributes $v+\tilde\Delta-C(s)$
with no bid and $\bar S+\xi-K-C(s)$ with a consummated bid; prices and premia net
out (Lemma~\ref{lem:transfer-netting}). Under Quiet Voice ($q=0$, $D=0$) the
order flow is $X=z$ and a bid arrives at cell $(X,0)$ with probability $p(X,0)$;
hence
\[
\Sigma_Q=v+\tilde\Delta-C(k_D)
 +\E_z\Bigl[p(X,0)\bigl(\bar S-K-v-\tilde\Delta\bigr)
 +\sigma_\xi\,\phi(T_{X,0})\Bigr],
\]
using the mean-zero Gaussian truncation identity
$\E[\xi\mid\xi\ge\sigma_\xi T]\,(1-\Phi(T))=\sigma_\xi\phi(T)$ cell by cell.
Under Public Voice ($D=1$) the disclosed branch is order-flow invariant
(Appendix~\ref{app:proof-disclosed-invariance}), so the same computation gives
$\Sigma_P=v+\tilde\Delta-C(k_D)
 +p_1^{\ast}(\bar S-K-v-\tilde\Delta)+\sigma_\xi\,\phi(T_1)$. Subtracting, with
$v=\E[v\mid s{=}k_D]$, gives \eqref{eq:reclass-real}.
...
threshold is the marginal type's bid \emph{lottery}: crossing $k_D$ flips $D$,
switching the type from the frequent, low-threshold quiet-branch bids
($\bar p_Q$) to the rare, high-threshold disclosed bids ($p_1^{\ast}<\bar p_Q$:
deterrence), so $\Sigma_P-\Sigma_Q\neq0$ even as the cutoff varies continuously.
At the baseline the jump is positive ($\approx+0.064$): the bids that disclosure
deters are value-\emph{destroying} at the margin (the marginal type's standalone
value $\E[v\mid k_D]+\tilde\Delta\approx1.86$ exceeds the raider's mean creation
$\bar S-K=1.29$, contributing $+0.19$), and this screening gain exceeds the
foregone $\xi$-selection ($-0.12$). Disclosure is socially valuable for the
marginal type because it \emph{screens out} low-quality control transactions, not
...
By \eqref{eq:reclass-real}, with deterrence ($\bar p_Q>p_1^{\ast}$, the baseline
configuration) the jump is positive iff
\begin{equation}
\bigl(\bar p_Q-p_1^{\ast}\bigr)
 \Bigl(\E[v\mid s{=}k_D^{\textup{eq}}]+\tilde\Delta-(\bar S-K)\Bigr)
 \;>\;\sigma_\xi\Bigl(\E_z\,\phi(T_{X,0})-\phi(T_1)\Bigr),
\label{eq:jump-positive}
\end{equation}
i.e.\ when the value destroyed by the quiet-branch bids that disclosure deters
--- the marginal type's standalone value $\E[v\mid k_D]+\tilde\Delta$ net of the
raider's mean creation $\bar S-K$, weighted by the deterred mass --- exceeds the
foregone $\xi$-selection. At the baseline both sides are computable from
\texttt{prices.csv}: $0.19$ against $0.12$, so \eqref{eq:jump-positive} holds.
It fails when the marginal control transaction is value-\emph{creating}
($\bar S-K$ large relative to the standalone value), in which case deterring
bids destroys surplus, the jump turns negative, and the conclusion is correctly
suspended.
...
$s$ and dictates $(q,a)$ subject only to feasibility engages quietly ($a=1$,
$D=0$) rather than not at all iff
\begin{equation}
\bigl(1-\bar p_Q\bigr)\,\tilde\Delta-C(s)\;\ge\;0,
\label{eq:fb-engage}
\end{equation}
(the improvement is realized only when no bid displaces the firm; premia are
transfers and do not enter), and prefers public disclosure to quiet engagement
iff the jump \eqref{eq:reclass-real} is positive. The first-best disclosure
cutoff $k_D^{\textup{FB}}$ solves
$\Sigma_P(k_D^{\textup{FB}})=\Sigma_Q(k_D^{\textup{FB}})$, i.e.\ the zero of the
jump.
\end{proposition}
...
pointwise in $s$. For a quiet type, engagement raises the no-bid branch by
$\tilde\Delta$ at cost $C(s)$ and leaves the bid-branch surplus $\bar S+\xi-K$
unchanged (the improvement is displaced by the control transfer; the premium is
a transfer by Lemma~\ref{lem:transfer-netting} and does not enter), giving
\eqref{eq:fb-engage}. Among engaging types, disclosure is preferred iff
$\Sigma_P(s)\ge\Sigma_Q(s)$; by single-crossing of $\Sigma_P-\Sigma_Q$ in $s$
--- under deterrence ($\bar p_Q>p_1^{\ast}$) the term $\E[v\mid s]$ enters
\eqref{eq:reclass-real} with the positive coefficient
$(\bar p_Q-p_1^{\ast})\beta$, so the jump is strictly \emph{increasing} in $s$
--- the optimum is a cutoff at the zero of the jump.
\end{proof}
...
Proposition~\ref{prop:disclosure-wedge}) and deterrence holds
($\bar p_Q>p_1^{\ast}$). Then:
\begin{enumerate}[label=\textup{(\alph*)},leftmargin=2em]
\item $k_D^{\textup{FB}}<k_D^{\textup{eq}}$: the first-best discloses strictly
more than the equilibrium.
\item If in addition (ii$'$) $G\ge0$ on $[k_D^{\textup{FB}},k_D^{\textup{eq}}]$
and $W$ is single-peaked, then
$k_D^{\textup{FB}}\le k_D^{\star}<k_D^{\textup{eq}}$.
\end{enumerate}
\end{corollary}
...
$k_D^{\textup{eq}}$, and since the jump is strictly \emph{increasing} in $s$
under deterrence (Proposition~\ref{prop:firstbest-d2}) its zero
$k_D^{\textup{FB}}$ lies at a strictly lower signal,
$k_D^{\textup{FB}}<k_D^{\textup{eq}}$.
...
by (ii$'$), which supplies non-negativity of $G$ at this point (this is why the
hypothesis is stated on the interval rather than at $k_D^{\textup{eq}}$ alone);
...
Everything in this section stress-tests the two headline results rather than adding a third. The polar full- and no-disclosure benchmarks bracket the disclosure-attenuation mechanism of Proposition~\ref{prop:disclosure-attenuation}, marking its upper and lower bounds; the noisy-rumor regime shows the same attenuation survives when partial observability arrives through media or leaks, not only through filings; and the general-equilibrium analysis shows where that attenuation ceases to carry a determinate sign once the blockholder re-optimizes her cutoffs. Each locates the boundary of the two established results rather than introducing a mechanism of its own.
...
\end{equation}
The decomposition itself is definitional (chain rule). The channel signs are carried as conditional readings, not theorems: the transparency effect---the partial derivative holding the equilibrium masses fixed---is nonnegative \emph{in the calibrated region}, where shifting mass to observable states ($D=1$) removes the inference discount (the general-equilibrium counterpart of the weight channel in Proposition~\ref{prop:disclosure-attenuation}); the deterrence channel collects the indirect terms---a stricter threshold forces earlier disclosure, raising bid deterrence and lowering the expected return to quiet engagement, which contracts the engagement mass $\omega_Q+\omega_P$---and is nonpositive \emph{whenever} the premium is increasing in the engagement mass and $d(\omega_Q+\omega_P)/d\tau\le 0$. The \emph{net} sign is indeterminate and governed by primitives. As with the liquidity comparative static, the indirect (cutoff-shift) terms admit no general closed-form sign---they require inverting the blockholder's cutoff Jacobian---so the trade-off is stated as an exact decomposition with conditionally signed channels rather than a signed net effect; no formal proof of the channel signs is claimed beyond the stated conditions.
\end{proposition}
...
Appendix~\ref{app:d8} upgrades this lemma's bound: the quantified contraction
modulus yields an inversion-free bound on $B(\kappa)$ requiring no path
derivative, a certified single-peakedness region at the baseline, and a
certified counterexample showing the hypothesis fails globally.
\end{remark}
...
\paragraph{Honest scope statement (updated).}
A full microfoundation must derive the threat-point shift $d(1)-d(0)>0$ (and hence Assumption~\textup{(A3)}, $\mtil>m_{0}$) from primitives. Within this appendix we do not do that: the sign and magnitude of the shift are governed by a pivotality/appropriability coefficient $\lambda$ that can only be pinned by a complete tender-game equilibrium. \textbf{That tender game is now solved in Appendix~\ref{app:d7}}, which derives $\lambda=1-q(1-\gamma)\psi$ from the disagreement-node environment and characterizes exactly when Condition~\ref{cond:bg} holds (Theorem~\ref{thm:d7-A3}). This appendix therefore retains its original role as the \emph{bargaining layer} --- everything proved here conditional on Condition~\ref{cond:bg} now operates on a derived premise --- and its honest-scope discipline applies one level down, to the institutional primitives of the tender environment listed in \S\ref{app:d7-ledger}. Following the strategy of \citet{EdmansGoldsteinJiang2015} and the corporate-finance tradition of stating results under transparent primitive conditions, we instead: (i) \emph{state} the threat-point shift as an explicit, economically interpretable condition (Condition~\ref{cond:bg}); (ii) \emph{motivate} that condition with a free-rider tender heuristic in the spirit of \citet{GrossmanHart1980}, flagged honestly as motivation rather than proof; (iii) prove what genuinely follows from the bargaining protocol \emph{given} the condition; and (iv) carry the one remaining numerical claim (the single-peak comparative static) as a \emph{numerically verified regularity} at the baseline calibration, clearly labelled as such.
...
This is a \emph{numerically verified regularity at the baseline calibration, not a theorem.} In the baseline, with the \emph{constant} wedge ($\lambda_{\mathrm{sd}}=0$), $\Delta^{\min}(\kappa)$ is single-peaked over $\kappa\in[0.01,0.99]$ with interior maximizer $\kappa^{\dagger}\approx0.59$, peak $\approx0.0776$, and amplitude $\approx0.0107$ ($\approx14\%$ of level). Under the state-dependent wedge \eqref{eq:bg-statedep-form}, the interior maximizer stays at $\kappa^{\dagger}\approx0.60$ and the profile remains single-peaked for \emph{every} $\lambda_{\mathrm{sd}}\in\{-0.4,-0.2,0,+0.2,+0.4\}$; the peak rises/falls monotonically with $\lambda_{\mathrm{sd}}$ from $\approx0.0722$ to $\approx0.0829$. All re-solves pass the solver's residual gate. The headline single-hump orientation is \emph{conditional} on the chord-curvature condition of the nonmonotonicity analysis; when that condition fails the profile inverts to a trough.
\end{numreg}

\subsection{What is proved, what is conditional, and what is open}
\label{app:bg-ledger}

For transparency: \emph{proved} are the closed-form Nash rent and participation (Lemma~\ref{lem:bg-split}), the $\xi$-invariance of the wedge \eqref{eq:bg-wedge}, the reduced-form nesting and converse (Proposition~\ref{prop:bg-reduced}), the $\rho^{2}$ event tree (Lemma~\ref{lem:bg-tree}), the level-shape decomposition (Proposition~\ref{prop:bg-levelshape}), and A3 \emph{conditional on} Condition~\ref{cond:bg} (Theorem~\ref{thm:bg-A3}). \emph{Conditional / numerical:} the threat-point shift is \emph{stated} as Condition~\ref{cond:bg} and only \emph{motivated} by the free-rider heuristic; $\theta$ is set-identified (Remark~\ref{rem:bg-ident}); and the single interior peak is a numerical regularity (Numerical Regularity~\ref{nr:bg-statedep}). \emph{Resolved since this record was written:} the appropriability coefficient $\lambda$ \emph{is} now derived from a complete tender-game equilibrium --- Appendix~\ref{app:d7} solves the disagreement-node game and delivers $\lambda=1-q(1-\gamma)\psi$ from primitives (Corollary~\ref{cor:d7-BG}), discharging what this ledger originally listed as open; the residual open margins of that derivation (bidder--fringe competition, repeated arrival, endogenous charter terms) are catalogued in \S\ref{app:d7-ledger}.
...
%==============================================================================
% Appendix D7: the disagreement-node tender game (derives Condition BG's
% lambda from primitives). Single source of truth: quality_reports/fixes/.
%==============================================================================
\input{quality_reports/fixes/D7_takeover_game_microfound.tex}
...
\end{equation}
At fixed prices $\Lambda(s)\equiv0$: Hold and Quiet share the trade ($q=0$) and the disclosure flag ($D=0$), so $Y_Q-Y_H$ is the engagement lottery, whose value is a cell-level object independent of the individual signal (eq.~\eqref{eq:UH-UQ}). The wedge therefore reduces to $W(s)=-C'(s)$. Under A2, $C'(s)=-(\chi/\sigma_s)\,C(s)<0$, so $W(s)>0$: the decreasing cost \emph{is} the single-crossing force on this margin. The substantive question is how the architecture behaves when $C'$ is flat or positive.

\subsection{The ordering proposition}
\label{app:d5-ordering}

\begin{proposition}[The exact role of A2 on the Hold/Quiet margin]\label{prop:d5-a2}
Suppose \textup{(A1)}, \textup{(A3)}--\textup{(A7)} hold and $P(\cdot)$ is fixed. Let $\Lambda(s)$ be as in \eqref{eq:d5-wedge}; at fixed prices $\Lambda(s)\equiv0$ (eq.~\eqref{eq:UH-UQ}: the gross Hold/Quiet gap is signal-free). Then:
\begin{enumerate}
...
\begin{itemize}
\item \textbf{persuasion-type} if $C'(s)<\Lambda(s)$ for all $s$ in the Quiet cell --- at fixed prices $\Lambda\equiv0$, so this is exactly $C'<0$, i.e.\ A2: the marginal cost of a louder voice falls with the strength of the case. The ordering of Proposition~\ref{prop:d5-a2} holds.
\item \textbf{entrenchment-type} if $C'(s)\ge\Lambda(s)\equiv0$ on a nondegenerate subset: the cost of engaging is flat or \emph{increases} in the signal (e.g.\ better-informed blockholders face harsher managerial entrenchment, litigation, or reputational retaliation precisely when their case is strong). Here $G_{HQ}$ can be non-monotone and the Hold/Quiet ordering may invert locally, producing a non-threshold (possibly disconnected) Quiet cell.
\end{itemize}
\end{definition}
...
Let $\Phi^{D0}(\kappa)$ denote the $D=0$ contribution to $\Delta^{\min}$. Under the two-point law $\{0,\bar\pi\}$, the $\pi=0$ atom carries the \emph{Exit} mass and the $\bar\pi$ atom the pooled $q=0$ mass: $\Phi^{D0}(\kappa)=\omega_E\,g(0)+(\omega_H+\omega_Q)\,g(\bar\pi)$, and the full minority object is $\Delta^{\min}=\Phi^{D0}+R(\kappa)$ with $R$ the $D=1$ (Public Voice) block. Separately, the tower identity $\E[\pi(a)\,\1\{D=0\}]=\omega_Q$ holds, where $\pi(a)=\1\{a=1\}$; it certifies the \emph{mass} $\omega_Q$ but not the integrand value $g(\bar\pi)$, and in general $(\omega_H+\omega_Q)\,g(\bar\pi)\neq\omega_Q$.
\end{lemma}
\begin{proof}
At the $\kappa$ limits the realized $D=0$ cells separate by trade direction: the Exit trade ($q=-1$) produces the $\pi=0$ atom with mass $\omega_E$, while Hold and Quiet ($q=0$) pool at the $\bar\pi=\omega_Q/(\omega_H+\omega_Q)$ atom with mass $\omega_H+\omega_Q$ (Lemma~\ref{lem:d5-endpoints}; Table~\ref{tab:example}'s cell map). The mass-weighted sum of $g$ gives $\Phi^{D0}$. By the tower rule $\E[\pi(a)\,\1\{D=0\}]=\PP(\textup{Quiet})=\omega_Q$ --- consistent, since $(\omega_H+\omega_Q)\,\bar\pi=\omega_Q$. The premium object and the mass do not coincide: $g(\bar\pi)=\bar m(\bar\pi)\,p(\bar\pi)\le\bar m(\bar\pi)<1$ at the calibration.
\end{proof}
...
\item \textbf{Interior peak (Lemma~\ref{lem:d5-endpoints}, Proposition~\ref{prop:d5-peak}, Conditional Claim~\ref{claim:d5-singlepeak}).} Endpoint symmetry $\Delta^{\min}(0^+)=\Delta^{\min}(1^-)$ is derived from the noise law plus cutoff convergence; the existence of an interior maximizer is unconditional (Weierstrass); strict single-peakedness is governed by the GE cutoff-shift channel (GE-dom): Appendix~\ref{app:d8} now certifies it as a theorem, up to $\varepsilon$-localization of the peak, on an interior region at the baseline ($\kappa^{\dagger}\approx0.59$, amplitude $\approx14$--$16\%$) and certifies its failure at the $\sigma_\xi=0.60$ cells (Proposition~\ref{prop:d8-counter}).
\end{enumerate}

%==============================================================================
% Appendix D8: the GE cutoff-shift channel -- region theorem + counterexample.
% Single source of truth: quality_reports/fixes/.
%==============================================================================
\input{quality_reports/fixes/D8_GE_dominance_MCS.tex}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{numerical_output/fig_ge_decomposition.pdf}
\caption{\textbf{The GE cutoff-shift channel (Appendix~\ref{app:d8}).}
Left: decomposition of $\mathrm{d}\Delta^{\min}/\mathrm{d}\kappa$ into the
posterior/pricing channel (A) and the GE cutoff-shift channel (B) along the
baseline equilibrium path; channel (A) dominates throughout the certified
region of Corollary~\ref{cor:d8-baseline}, so the hump is a theorem there
(up to $\varepsilon$-localization of the peak, Theorem~\ref{thm:d8-region}).
Right: hump/trough classification over the $(\bar S,\sigma_\xi)$ robustness
family. The troughs at $\sigma_\xi=0.60$ are the certified counterexamples of
Proposition~\ref{prop:d8-counter}: channel (A) is single-peaked in every cell,
so the trough orientation is produced entirely by the GE channel.}
\label{fig:ge-decomposition}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{numerical_output/fig_wedge_primitives.pdf}
\caption{\textbf{Minority gains under the microfounded wedge
(Appendix~\ref{app:d7}).} Equilibrium $\Delta^{\min}(\kappa)$ when the premium
wedge is generated by the disagreement-node tender game,
$m_1-m_0=(1-\theta)\lambda\rho\Delta_{\mathrm{eng}}$ with
$\lambda=1-q(1-\gamma)\psi$. Left: sweeping the portability $\gamma$ of the
activist's improvement under fringe acquirers ($\gamma=0$: superseding
acquirers erode appropriability). Right: sweeping the fringe-raid intensity
$q$. The legend reports the implied appropriability $\lambda$; the sensitivity
analysis of Figure~\ref{fig:sensitivity-wedge} is thereby re-expressed in
observable primitives.}
\label{fig:wedge-primitives}
\end{figure}
