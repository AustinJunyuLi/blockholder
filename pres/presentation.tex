...
% 40-minute research talk: 18 core slides + backup slides with
% bidirectional hyperlinks (technical material lives in the backups)
% ============================================================================
%
...
% ── Theme: Metropolis (modern, minimal), accents matched to the paper's
%    figure palette (Paul Tol muted; see pyfig/style.py) ──────────────────
\usetheme[progressbar=frametitle, numbering=fraction, block=fill]{metropolis}

% Palette
\definecolor{accentblue}{HTML}{4477AA}   % sensitivity blue (pyfig)
\definecolor{accentteal}{HTML}{44AA99}   % Public Voice teal (pyfig)
\definecolor{accentrose}{HTML}{CC6677}   % Exit rose (pyfig)
\definecolor{inkdark}{HTML}{23373B}      % metropolis dark slate
\colorlet{brightblue}{accentblue}        % legacy alias used throughout the deck

\setbeamercolor{progress bar}{fg=accentblue, bg=accentblue!25}
\setbeamercolor{title separator}{fg=accentblue}
\setbeamercolor{alerted text}{fg=accentrose}
\setbeamercolor{example text}{fg=accentteal}
\setbeamercolor{frametitle}{bg=inkdark, fg=black!2}

% Title page / frametitle typography
\setbeamerfont{frametitle}{series=\bfseries}
\metroset{titleformat frame=regular, titleformat title=regular}
% Tighter frame-title bar (reclaims vertical space on dense frames)
\makeatletter
...
% Beamer cosmetics (metropolis already drops navigation symbols and sets a
% clean frame-number footer via numbering=fraction)
\setbeamerfont{block title}{series=\bfseries}

% Graphics path: slide-only variants in figures/, canonical manuscript
% figures in ../numerical_output/, empirical facts in ../empirics/output/
\graphicspath{{figures/}{../numerical_output/}{../empirics/output/}}
...
\begin{itemize}\itemsep0.35em
  \item \textbf{\citealp{JohnsonSwem2021}} (JFE): dynamic \emph{reputation} --- the model's only state variable; accumulation collapsed to a lognormal cost draw; no trading, no disclosure margin.
  \item \textbf{\citealp{AlbuquerqueFosSchroth2022}} (JFE): structural 13D-vs-13G choice; engagement cost a \emph{scalar}; 13D returns $\approx 3/4$ treatment --- the market \emph{capitalizes} activism at disclosure.
  \item \textbf{\citealp{CelentanoLevine2025}}: equilibrium activism $+$ takeovers $+$ discipline; estimate activism \emph{lowers} bid premia $13.7\%$ via board bargaining; no market microstructure.
\end{itemize}

\begin{block}{This paper's lane}
\small Liquidity, order-flow inference, and the disclosure rule are the trio's \textbf{missing state variables}. The premium they estimate as a constant is here an \textbf{equilibrium object of tender mechanics} ($\lambda = 1-q(1-\gamma)\psi$) --- and the negative activism--premium estimate is the low-$\lambda$ region of our model, with cross-sectional content ($q,\gamma,\psi,\kappa$) their board-bargaining channel cannot index.
\end{block}
\end{frame}
...
\begin{itemize}\itemsep0.1em
  \item \textbf{US:} 5\% threshold (Schedule 13D); window cut
        \textbf{10 calendar $\to$ 5 business days} (eff.\ Feb 2024)
...
{\scriptsize\textbf{Scope:} the GE clause holds as a \emph{theorem} on a certified region, with a certified counterexample at its boundary (Theorem B).}
...
% ── SLIDE 10c: Theorem A — the wedge from tender mechanics ───────────────
\begin{frame}{Theorem A: The Premium Wedge from Tender Mechanics}
...
\vspace{0.2em}
{\footnotesize\textbf{Negative premium estimates} (\citealp{CelentanoLevine2025}) \textbf{reconciled}: $\bar m/P$ falls in activism evidence when $\lambda$ is small.}

\backuplink{backup:tender_game}{The tender game}
\end{columns}
\end{frame}

% ── SLIDE 10d: Theorem B — certified region + counterexample ─────────────
\begin{frame}{Theorem B: When Is the Hump a Theorem?}
...
\backuplink{backup:region_method}{How the region is certified}
\end{columns}
\end{frame}
...
% ── SLIDE E1: Evidence I — the 2024 acceleration bit (Fact 1) ────────────
\begin{frame}{Evidence I: The 2024 Acceleration Bit}
\hypertarget{main:fact1}{}
\begin{columns}[T,onlytextwidth]
\column{0.55\textwidth}
\centering
\includegraphics[width=0.96\linewidth]{fact1_delay.pdf}

\column{0.43\textwidth}
{\small\textbf{Rule change:} SEC release 33-11253 cut the Schedule 13D
initial deadline from \textbf{10 calendar days} to \textbf{5 business days}
(compliance 2024-02-05).}

\vspace{0.3em}
\begin{block}{Fact 1: Filing delays compressed}
{\small Median event-to-filing delay falls \textbf{7 $\to$ 5} business days;
the share filed within 5 business days more than doubles:
\textbf{36\% $\to$ 76\%}.}
\end{block}

\vspace{0.2em}
{\footnotesize EDGAR originals, 2023Q2--Q3 vs.\ 2024Q3--Q4 --- the model's
quiet-trading cover, shortened by law.}

\backuplink{backup:fact_methods}{Data \& methods}
\end{columns}
\end{frame}

% ── SLIDE E2: Evidence II — 13D announcement returns (Fact 2) ────────────
\begin{frame}{Evidence II: Returns on the Modeled Margin}
\hypertarget{main:fact2}{}
\begin{columns}[T,onlytextwidth]
\column{0.55\textwidth}
\centering
\includegraphics[width=0.96\linewidth]{fact2_car.pdf}

\column{0.43\textwidth}
{\small\textbf{Design:} market-model CARs around original 13D filings,
2022--2025; 1{,}513 events (843 pre, 670 post); \emph{Post} $=$
5-business-day regime.}

\vspace{0.2em}
\begin{block}{Fact 2: What the data say}
{\small Mean $\mathrm{CAR}[-1,+1]$ \textbf{doubles}: $0.85\% \to 1.71\%$
($\beta > 0$ as predicted; $t \approx 1.1$, n.s.).\\[0.2em]
Surprises concentrate in \textbf{liquid} names:
$\partial\mathrm{CAR}/\partial\ln\mathrm{Amihud} = -1.4$pp
($t = -3.7$) --- the camouflage margin is first-order.\\[0.2em]
Post$\times$illiquidity interaction: precise null.}
\end{block}

\vspace{0.2em}
{\footnotesize Two-way clustered (firm, month); table in backup.}
...
The premium wedge is derived from a free-rider tender game
($\lambda = 1-q(1-\gamma)\psi$; rationalizes the estimated negative
activism--premium effect), and the GE clause of the hump is certified as a
theorem on $[0.35,0.825]$ with a genuine counterexample at $\sigma_\xi = 0.60$.
...
Backup: Mathematical Foundations \&\\ Equilibrium Robustness
\end{frame}
...
% ── BACKUP 6e: The tender game behind Theorem A (D7) ───────────────────
\begin{frame}{What game pins the premium wedge?}
\hypertarget{backup:tender_game}{}
\small
\textbf{Disagreement-node tender game.} Players: fringe raider (entry cost
$c_F \sim H$, synergy $S_F \sim G$), bloc (stake $\alpha$, strategic),
atomistic float (measure $1-\alpha$); charter permits dilution $\varphi$ of
non-tendering shareholders (Grossman--Hart exclusion device).

\vspace{0.25em}
\textbf{Timing:} raider enters and posts a uniform unrestricted offer
$\hat b$ for control threshold $\tau$ $\to$ float tenders atomistically
(free-rides) $\to$ control transfers iff tenders $\ge \tau$; post-raid
per-share value $w + \gamma\, a\,\Delta_{\text{eng}} + S_F$.

\vspace{0.25em}
\textbf{Equilibrium} (with $\alpha < \tau$: the bloc alone cannot deliver
control): free-rider floor $\hat b^* = w + \gamma a \Delta_{\text{eng}} + S_F
- \varphi$; a \emph{pivotal} bloc ($1-\alpha < \tau$) blocks floor raids
unless $S_F - \varphi \ge (1-\gamma)\,a\,\Delta_{\text{eng}}$. On the
blocking set the raider can still top up to the bloc's reservation --- but
such raids pay the bloc \emph{exactly} its outside option, so the threat
point (and $\lambda$) is unchanged.

\vspace{0.25em}
\begin{block}{Appropriability as an equilibrium output}
\vspace{-0.4em}
\[
\lambda = 1 - q\,(1-\gamma)\,\psi, \qquad
\psi = \tfrac{1}{(1-\gamma)\Delta_{\text{eng}}}
\textstyle\int_0^{(1-\gamma)\Delta_{\text{eng}}}
\PP(S_F - \varphi \ge t)\,dt \in [0,1]
\]
\vspace{-0.9em}
\end{block}

{\footnotesize ($\psi = 1$ when non-pivotal.) $\lambda > 0$ fails \textbf{only if}
$q=1$, $\gamma=0$, and blocking is impossible or worthless --- so A3 is a
theorem, not an assumption. Wedge: $m_1 - m_0 = (1-\theta)\lambda\rho
\Delta_{\text{eng}}$. Residue: one-shot fringe, exogenous $(\varphi,\tau)$.}

\returnlink{main:thmA}{Theorem A}
\end{frame}

% ── BACKUP 6f: How the GE region is certified (D8) ─────────────────────
\begin{frame}{How is the certified region actually certified?}
\hypertarget{backup:region_method}{}
\small
\textbf{Problem:} the hump needs the GE cutoff-shift channel $B(\kappa)$ not
to overturn the fixed-cutoff channel (A). Bounding $B$ requires
$\|\mathrm{d}k/\mathrm{d}\kappa\|$ --- normally the \emph{unknown} equilibrium path derivative.

\vspace{0.3em}
\textbf{Inversion-free bound:} the cutoff map $F = I - T$ inherits the
A6 contraction modulus $L$ of the best-response map $T$, so by the Neumann series
\[
\left\|(I - D_k T)^{-1}\right\|_\infty \le \frac{1}{1-L}
\quad\Longrightarrow\quad
\left\|\frac{\mathrm{d}k}{\mathrm{d}\kappa}\right\|_\infty \le
\frac{\|\partial_\kappa T\|_\infty}{1-L},
\]
computable from \emph{one} equilibrium solve plus one finite-difference column
--- no Jacobian inversion, no path knowledge.

\vspace{0.3em}
\textbf{At the baseline:} $L \le 0.836 < 1$ along the whole path; channel-(A)
peak at $\kappa \approx 0.60$; certification margins hold at every off-ball
grid point (ball integral $2.5\times10^{-5}$ vs.\ margin $4.7\times10^{-3}$)
$\Rightarrow$ exact-IFT region $[0.30, 0.85]$, inversion-free region $[0.35, 0.825]$.

\vspace{0.3em}
{\footnotesize\textbf{And the counterexample is certified too:} at
$\sigma_\xi = 0.60$, troughs at interior $\kappa \in \{0.275, 0.35, 0.475\}$
with channel (A) single-peaked in every cell and $\int |B|$ exceeding the
(A)-amplitude (e.g.\ $0.021$ vs.\ $0.0069$) --- GE dominance is genuinely
non-removable.}

\returnlink{main:thmB}{Theorem B}
\end{frame}

% ── CATEGORY DIVIDER: Sensitivity & Comparative Statics ──────────────────
\begin{frame}[standout]
Backup: Sensitivity \& Comparative Statics
\end{frame}
...
\begin{frame}[standout]
Backup: Disclosure Extensions \&\\ Information Regimes
\end{frame}
...
\begin{frame}[standout]
Backup: Calibration \& Numerical Details
\end{frame}
...
\textbf{Key:} (i) $D{=}1$: $\pi = 1$, flat prices; (ii) $D{=}0$: inference gradient; (iii) $D{=}1$ has \emph{much lower} bid probability --- disclosure deters bids through the price run-up and the full premium $\tilde m$.
...
% ── CATEGORY DIVIDER: Data & Evidence ────────────────────────────────────
\begin{frame}[standout]
Backup: Data \& Evidence
\end{frame}

% ── BACKUP 21: Facts 1–2 data & methods ─────────────────────────────────
\begin{frame}{How are the two facts constructed?}
\hypertarget{backup:fact_methods}{}
\footnotesize
\textbf{Sample.} All \emph{original} Schedule 13Ds on EDGAR (amendments
excluded), full-text indexes 2022Q1--2025Q4. Form-type matching is
boundary-aware: EDGAR renamed the form \texttt{SC 13D} $\to$
\texttt{SCHEDULE 13D} in Dec.\ 2024, and naive fixed-width reads conflate
originals with amendments (and would drop 2025 entirely).

\vspace{0.25em}
\textbf{Fact 1 (delays).} Event date from the cover-page label (or the
structured XML tag in the post-2024 regime); delay $=$ business days from
event to filing; windows 2023Q2--Q3 vs.\ 2024Q3--Q4 straddle the 2024-02-05
compliance date.

\vspace{0.25em}
\textbf{Fact 2 (CARs).} Subject CUSIP parsed from cover pages with
check-digit validation and label-proximity voting ($\approx$80--90\%
coverage); filings accepted after 16:00 ET shift to the next trading day.
Market-model abnormal returns (CRSP daily; estimation 220 trading days
ending 30 before the event, min.\ 100 obs); $\mathrm{CAR}[-1,+1]$ and
$\mathrm{CAR}[-10,+1]$.

\vspace{0.25em}
\textbf{Covariates \& inference.} Amihud illiquidity over $[-250,-30]$,
run-up $[-60,-11]$, $\ln$ market cap at $-30$; US common stock only
(CIZ share-class filter); OLS with CGM two-way clustered SEs (firm $\times$
calendar month); year-quarter FE absorb \emph{Post} in the saturated spec.

\returnlink{main:fact2}{Evidence II}
\end{frame}
