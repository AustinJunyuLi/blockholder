...
% sections_v3/proofs_*.tex; numerical records from research/model_v4/HANDOFF_sign.md
...
\date{This draft: 29 August 2026}
...
dominance-and-contraction region carried as a named hypothesis, with grid evidence that such
nodes exist, including a contiguous block of nine nodes. On the implemented calibration the window record shows
attenuation at every checked node, with the composition leg carrying the effect. The empirical
anchor is the February 2024 acceleration of the Schedule 13D deadline from ten calendar days to five
business days: executed descriptives show the median filing delay falling from seven to five
business days and the share filed inside the new deadline rising from 36 to 76 percent. The paper
closes with a pre-specified empirical design (a run-up/filing-day timing split, a bindingness
dose, and a bounded null built from the Commission's own tables), and with the choice of headline
empirical claim left open, not yet made.
\end{abstract}
...
filing lands; the February 2024 acceleration cut it in half. Neither margin is scenery. Both move
...
\citep{KahnWinton1998}, liquidity disciplines through exit \citep{AdmatiPfleiderer2009}. The
...
\citet{KyleVila1991} mix disclosed and undisclosed trades but disclaim disclosure requirements by
assumption; \citet{BackEtAl2018} have a fixed horizon at which the stake becomes known and show that
...
both on path, the liquidity sensitivity of the expected engagement-related premium factors exactly
...
rate in the threshold margin. The region is not exhibited; what is on file is numerical evidence
...
and the composition leg carries almost all of it; the weight leg shaves 3 to 14 percent. At that
...
stay proved, their proofs untouched; at that calibration they say nothing about the implemented
pooled cell. The paper says this once, where it belongs, and the spec for the empirics carries both
possible signs rather than assuming the theory's direction.
...
three legs: a \emph{timing split} that tests the partition directly (the run-up from the 5 percent
crossing to the filing should move with liquidity, the filing-day jump should not, and the two
slopes differ); a \emph{bindingness dose} that splits filers by how hard the new deadline binds
them; and a \emph{bounded null}, arithmetic on the Commission's own tables that caps the
accumulation channel's effect on the twelve-month bid hazard at about three percentage points, a
ceiling the matched difference-in-differences cannot separately detect, which is stated as the
design's honest deliverable rather than a footnote. The choice of headline empirical claim is one I
have not made; Section~\ref{sec:headline-shell} lays out the candidates and marks the choice
pending.
...
empirics, and Section~\ref{sec:conclusion} concludes. The proofs sit in
Appendices~\ref{sec:proofs-core}, \ref{sec:proofs-existence} and~\ref{sec:proofs-ge}.
...
control-intent filers use the 13D and passive investors the shorter 13G, and which of the two a
given block lands in is itself a choice the rule frames.
...
therefore a cross-section object and the window margin a time-series one, and the February 2024
acceleration is the cleanest dated change either margin has had in half a century, which is why
...
stock without the flag. Second, the pooled cell is pooled \emph{for the price-setting market}.
...
filings by engaging investors in their sample land in the eight-to-ten-day bucket and roughly a fifth on day ten
itself. So the window binds for a large share of filers, which is what makes the February 2024
...
technology, and the others abstract from control transfers by name. The object this paper studies,
...
position, so in equilibrium the engaging blockholder's stake sits at the threshold), and they assert, without
...
are unusually likely to be acquired, which is the fact that makes bidder entry the natural control
...
consumes, and three of them carry numerical records showing that they fail at the calibration
the accompanying implementation runs. Those records are stated with the results that consume them
(Remarks~\ref{rem:A3record}, \ref{rem:A6record}, \ref{rem:AtauRecord} and~\ref{rem:P1record}), not
deferred.
...
against a $10^{-9}$ payoff tolerance. The pointwise argmax runs $H,V,H,V$, single-valued on each
...
candidate mechanical account of the four unresolved nodes (a fixed point sitting exactly on an
edge where $U_H - U_V$ jumps through zero without crossing) has been swept over all four under a
pre-registered rule and holds at all of them. Under the alternative, mass-proportional off-path
...
claimed nor denied. None of this changes any result's status: Assumption~\ref{asm:A3} is a hypothesis, and
Proposition~\ref{prop:P1} stays proved as a conditional.
\end{remark}
...
clause carries adverse applicability evidence of its own. None of that changes any result's
status: Assumption~\ref{asm:A5} is a hypothesis, and no result below consumes its cutoff clause.
...
vanishing-mass defusal is therefore refuted. The largest-weakly-increasing-selection tie-break is
...
pooled histories) on which no family works at all. At the implemented calibration the failure is
...
$k_2 = 1.659062163$. None of this changes any result's status: A6 is a hypothesis, and
Proposition~\ref{prop:P1} stays proved as a conditional. Nonexistence is neither claimed nor shown:
$23$ of $27$ sweep nodes converge, and a discontinuous self-map may still have fixed points. A
standard repair (a $t$-constrained game with a Kakutani argument and $t \downarrow 0$) is
identified but not executed, and what the implementation ships is a hard switch, so the
$\Theta$-continuity that repair would buy is absent from the implementation at every resolvable
scale.
\end{remark}
...
$C_h(\bar\pi) \le 0$ with $|C_h|$ weakly increasing in $\bar\pi$. Two further clauses are part of
the restriction:
...
\item \emph{$\kappa$-localisation.} At fixed policies all $\kappa$-dependence of $M_P$ sits in the
  ...
  composition-through-$\kappa$ remainder. The trailing ``hence'' is derivable, not assumed.
...
Theorem~\ref{thm:T1}, and by nothing else. Clause (br-v) was independently required by three
separate verification passes, one of which confirmed that it is not implied by (br-i)--(br-iv). One
sharpening is recorded rather than assumed: with $\varrho := \tfrac12 A_{1/2} + A_1$, which is provably
$\kappa$-free, $\bar\pi = \bar\pi_{\mathrm{pr}}/\varrho$, so (br-iv) is equivalent to
$\varrho(\tau')/\varrho(\tau) \ge \bar\pi_{\mathrm{pr}}(\tau')/\bar\pi_{\mathrm{pr}}(\tau)$; under the
level-symmetric reading $\varrho = \tfrac12$ and $\bar\pi = 2\bar\pi_{\mathrm{pr}}$, which forces
...
The \emph{outer cutoff best-response map} is $\Tmap(k;\vartheta)$, a continuous self-map of $\Theta$; it
...
in Appendix~\ref{sec:proofs-ge}. The \emph{dominance-and-contraction region} is $\mathcal R_r$, with
...
making. Its hypothesis set is long, and every clause of it is load-bearing (an earlier
statement was withdrawn and re-verified over exactly which form of one hypothesis the argument
...
\item Assumption~\ref{asm:A7J}, \emph{joint tuple injectivity}: the map
  ...
  form, so the two verification passes on file covered two different statements; the earlier statement
  was withdrawn and re-verified on that account, and the statement here is the amended one,
  checked by a fresh adversarial proof-read and an independent statements-only re-derivation,
  both by readers who did not write the proof. The form is named every time either appears.}
...
plans --- and the upper-set engagement-flag hypothesis (the engagement flags $a_j$ equal one on an
upper set of the ordered menu). \emph{Uniqueness is not claimed.}
\end{proposition}
...
The proof (Appendix~\ref{sec:proofs-existence}) builds the equilibrium object rather than
...
verifies it is a continuous self-map of the compact ordered polytope $\Theta$ under the reading of
Assumption~\ref{asm:A6}, applies Brouwer's fixed-point theorem, and assembles the six requirements
of Definition~\ref{def:cpbe} at the fixed point, with the boundary values of $\kappa$ handled by
extension. Part~F is the addendum: at an equilibrium at which Assumption~\ref{asm:A8} holds, both
cells are on path. The full proof carries a closing scope statement of six disclaimers:
uniqueness in no form, the addendum conditional at the equilibrium and nowhere else, boundary
extension, one fixed convention, flagged-order optimality without uniqueness, and the withdrawn
claim of a belief at every pooled history at $\kappa = 1$.
...
nothing about the implemented model. Nonexistence is neither claimed nor shown anywhere. The label
rests on the proof and its two verification passes, not on a grid, and the grid evidence is stated
separately for that reason: twenty-three of twenty-seven equilibrium-sweep nodes converge, and the
remaining four ($\kappa \in \{0.15,\,0.85\}$ crossed with $(\tau,T) \in \{(0.05,\,5),\,(0.075,\,
1)\}$) stay unresolved after thirty seeds each, with payoff-scale residuals of $10^{-4}$ to
$10^{-3}$ against a $10^{-9}$ criterion. Existence at those four nodes is neither claimed nor denied
by that evidence.
\end{remark}
...
\end{enumerate}
\end{lemma}
\resultstatus{Proved under the stated hypotheses (Appendix~\ref{sec:proofs-core}). None of the
hypotheses this lemma consumes carries adverse numerical evidence at the implemented calibration.}
...
\end{lemma}
\resultstatus{Proved under the stated hypotheses (Appendix~\ref{sec:proofs-core}). The executed
...
is the one the restriction is about: an independent re-enumeration reproduces the pooled mass to
...
numerical applicability evidence at one calibration; none of it changes any result's status,
because $\Atau$ is an assumption rather than a labelled claim. Lemma~\ref{lem:L3}, leg 3 of
Lemma~\ref{lem:L4} and Part~(B) of Theorem~\ref{thm:T1} stay proved as conditionals with their
proofs untouched; at this calibration they say nothing about the implemented pooled cell. The open
...
\end{lemma}
\resultstatus{Proved: legs 1--2 outright, leg 3 under $\Abr$ (Appendix~\ref{sec:proofs-core}). Legs
...
claimed.}
\end{theorem}
\resultstatus{Proved at fixed policies (Appendix~\ref{sec:proofs-ge}). Part~(B) is conditional on
...
\label{rem:T1record}
The executed check runs the two-round model at frozen policies, cutoffs pinned at the baseline
...
$(T',T) = (5,10)$. Attenuation holds at every node: $W_T C_T < 1$ throughout, and the composition
leg carries the effect: $C_T$ runs from $0.21$ to $0.79$ while $W_T$ shaves only $3$ to $14$
percent.

\begin{table}[H]
\centering
\caption{The window-margin record at fixed policies: cutting the window from $T=10$ to $T=5$ at the
implemented calibration. Weight leg $W_T$, composition leg $C_T$, product $W_T C_T < 1$ (attenuation)
at every $\tau$ node. Verified on the numerical grid; sensitivity measured as the total variation of
$\Dact$ over the $\kappa$ grid. The $H = 12$ column re-runs the comparison with $T = 10$ strictly
interior to the horizon, where the $0.9$-quantile node returns exactly $1.0$ because the $\tau$
ladder stops biting, not because the product crosses one.}
...
no mass at all and returning exactly $1$. The factorisation of Part~(A) holds to machine noise,
...
the $H = 12$ column of Table~\ref{tab:window-record}, where $T = 10$ is strictly interior, confirms
attenuation. What the record does not do is supply a sign theorem: it is node evidence at one
...
A separate executed comparison is sometimes read as evidence about the window, and it is not one. In
the one-round predecessor model, with cutoffs frozen at its baseline, the expected engagement-related
premium can be computed twice: once with the market seeing the disclosure flag alongside order
flow, once with order flow alone. The ratio of liquidity sensitivities (flagged divided by pooled,
total variation over $\kappa \in [0.15,0.85]$) is $1.064$, $1.184$ and $1.136$ at flagged weights
$\Omega = 0.037$, $0.129$ and $0.286$, falling to $0.378$ at $\Omega = 0.50$, with the sign crossing
...
($\Omega = 0.037$). The upper curve prices the blockholder's engagement-driven premium with the
market seeing order flow and the disclosure flag; the lower curve prices it from order flow alone.
...
hypothesis (Appendix~\ref{sec:proofs-ge}); the region itself is not exhibited, and the node evidence
...
check, independently re-run, on which all values reproduce. What these nodes verify is narrow and
...
dominance-and-contraction node is a pointwise numerical fact, not a fifth honesty category. A
...
re-derivation on file adds the bound $\mathcal B_r^{GE} = O\bigl((1-L_{\mathcal R})^{-3}\bigr)$, so
...
\item \emph{Whether the continuity clause of Assumption~\ref{asm:A6} holds for the declared
  construction of $\Theta$.} This was answered in substance: the mechanism is real, the locus is
  the cell-edge hyperplanes and sole-generator collapse faces, and the implemented menu's
  Hold-collapse face is measured clean. What remains open is scoped in
  Remark~\ref{rem:A6record}: whether a constructive $\Theta$, or a $t$-constrained Kakutani route,
  replaces the continuity half of the hypothesis in the statement (the repair is identified, not
  executed); the complementary menu class where the collapse-face clause may be satisfiable; and
  nonexistence, which is neither claimed nor shown anywhere.
\end{enumerate}
...
from filings on file, with the sample stated exactly and no standard errors claimed. The
specification in Section~\ref{sec:spec} has \emph{not} been run: it is written down in full
(variables, windows, samples, standard errors, placebos, and what would count for and against)
before any estimate exists. The headline claim is a choice not yet made;
Section~\ref{sec:headline-shell} lays out the candidates and marks the choice pending.
...
acceleration. Seeded samples of initial 13D filings, parsed from EDGAR with the project parser
Delay in business days, holiday-adjusted. Of the $300$ sampled filings, the delay statistics are
computed on the $188$ whose trigger date parses ($98$ pre, $90$ post); at parse rates of $0.68$ pre
and $0.64$ post, every count is a floor. Descriptive statistics; no standard errors are claimed.}
\label{tab:fact1}
...
deadline more than doubles, from 35.7 to 75.6 percent. Second, the pre-rule distribution was already
bimodal in the way \citet{BebchukBravJacksonJiang2013} document on filings by engaging investors: a large
fast group, a bunching group at the deadline, and a tail of late filers. The acceleration bit
a population that was heterogeneous in exactly the dimension the rule targets; the design below
exploits that heterogeneity as a dose. Third, the numbers agree in kind with the Commission's own
...
not the universe; the parse rates are floors; and these are descriptives, with no standard errors,
because none are claimed. The delay is counted in business days throughout; counting calendar
...
2024-02-08 belonged to the old regime. Filings that straddle are excluded and counted. The pre
window ends at the adoption date (2023-10-10) rather than the effective date, so the
adoption-to-effect stub is its own reported bin and anticipation is not pooled into ``pre''. The
EDGAR dissemination cut-off moved from 5:30 pm to 10:00 pm ET on the same date the window changed,
which shifts the flagged day by one for part of the post sample; the effective filing date FD$^{*}$
is constructed to absorb this, and re-running with FD$^{*} \equiv$ FD is a required robustness row.
A full re-fetch and re-parse with the current parser is a stated precondition for any estimate, and
...
whether the run-up's liquidity slope changed after February 2024, on a fixed five-day window from
...
it. The dose is a filer attribute built from pre-period behaviour only (the share of the filer's
pre-period initial 13Ds whose delay exceeded five business days, in $[0,1]$), so it cannot be
contaminated by the reform, with a binary median-delay split reported alongside. The dose
specification interacts the dose with Post and clusters on filer as well. Its first stage is
mechanical (a fast filer cannot get faster) and is labelled a validity check, not a finding; the
only published estimate of the same first stage is \citet{Trivedi2026}'s compliance share, $+0.348$
...
within twelve months against 7.2 percent of matched controls \citep{GreenwoodSchor2009}. The
design's minimum detectable effect is $4.4$ percentage points, \emph{above} the 3-point ceiling of
the bounded null, so the difference-in-differences cannot separate the accumulation effect from
zero even at the ceiling, and that sentence, not a point estimate, is the leg's honest deliverable.
An estimate larger than both the detectable effect and the ceiling would be attributed to
incumbent defence or selection, with a diagnostic offered for which.

\paragraph{(f) The referee checklist.}
...
and the incumbent-defence channel, which runs opposite to accumulation and whose proposed moderator
(charter pill provisions) is a heterogeneity split the design can build. Power: computed from
counts on disk plus stated variance assumptions, with the arithmetic shown; the timing split
...
a $p$-value below $0.10$ removes causal language from the leg. Parser validation: an executable test
suite, with the two stated gaps (the committed pipeline output is pre-XML; the full-universe
re-parse has not run) reported rather than papered over.
...
points), which is a reportable outcome rather than a failure. Scope: it is the candidate most
...
theorem separates the rule's two margins honestly: the threshold margin attenuates without a
dominance condition, the window margin only if and only if a weight ratio times an unsigned
composition ratio stays below one. The general-equilibrium survival of the fixed-policy sign is an
...
The empirics rest on the February 2024 acceleration. The executed fact is that filers moved: the
...
partition directly, a bindingness dose, a bounded null from the Commission's own tables, a matched
design on the bid hazard whose honest deliverable is already known to be a ceiling. The
...
whether the existence hypotheses can be repaired constructively on the class of menus the
implementation uses; and whether the disclosed share of engagements (the calibration target
$\omega_a$, which no dataset in the literature can see) can be bounded tightly enough to place
the world on one side of the composition sign. Each is a question about the domain of an assumption
...
$j^\star$, $\mathfrak S(k)$, $\mathfrak w$, $\mathcal Q_j(s)$, $G_j$, $E_j$, $\mu_n$, $L_j$, $w_n$,
$t_n$, $Z_n$, $\Lambda_k$, $\Lambda_u$, $(\hat v_\circ,\pi_\circ)$, $P_\circ$,
$\operatorname{supp}(z_d)$, $s_F$) are declared where they are first used. The selection set
$\mathfrak S(k)$ is written in fraktur so that it does not collide with the liquidity sensitivities
$\mathcal S$ and $\mathcal S_P$ of Definition~\ref{def:premium}. The engagement cost
...
part of the claim. One withdrawal is recorded with it: the Kakutani conclusion sketched in the
preceding sentences is \emph{unestablished}, because convex values and a closed graph of
$\mathfrak T$ would need a lemma this proof does not supply. No Kakutani conclusion is drawn; the
step stays a remark outside the claim, no step reads it, and nothing the proposition rests on
changes.
