...
%==============================================================================
\begin{abstract}
\noindent Disclosure rules change what a market knows while an investor is building a stake. I study
how that timing affects bidder entry and the takeover premium. In the model, a blockholder first
trades without a public flag. If her stake crosses the legal threshold and the filing window
expires, the market sees her stake and intent; otherwise it sees only order flow. This split gives
the threshold and the filing window different economic roles. At fixed trading policies, a tighter
threshold reduces the sensitivity of the engagement-related premium to liquidity. A shorter window
does so only when the change in who remains hidden is not too large. The model therefore gives no
unconditional sign for the 2024 US acceleration. Seeded filing data show that median delay fell
from seven to five business days. I set out a pre-specified design to test the market and takeover
implications.
\end{abstract}
...
two-round model. The empirical design keeps attenuation and amplification as live possibilities.

The first descriptive fact is already visible in the filing data. In a seeded sample, the median
trigger-to-filing delay fell from 7.0 business days before the February 2024
acceleration to 5.0 afterwards, and the share
filed within five business days rose from 35.7 to 75.6 percent. The remaining empirical work is a
written specification rather than a set of estimates. It tests the run-up and filing-day jump
separately, uses pre-period filing behaviour as a bindingness dose, studies the stake at filing, bounds
the accumulation channel's effect on the bid hazard, and estimates bidder entry by liquidity. The
specification reports both signs and treats the preliminary sample as descriptive.
...
descriptives and the pre-specified tests. Section~\ref{sec:conclusion} closes. The proofs sit in the
online appendix.
...
model counterpart of the timing split the empirics of Section~\ref{sec:empirics} measure: the total
...
acceleration is attenuation, and the empirics of Section~\ref{sec:empirics} treat that as one of
two live branches, not as a derived sign.
...
the identity \eqref{eq:runup-jump} and the flagged cell's $\kappa$-invariance turned into
...
on the month dimension and the more conservative of the two reported. The February 2024
acceleration test (H2) asks
...
cross-section of delays is therefore never read as a window effect; only the 5 February 2024
filing-window change is. The historical
...
\end{equation*}
The coefficient $\tau$ asks whether the effect of the February 2024 acceleration on bidder
entry differs by liquidity. A
...
as a bounded null. Placebos use all 568 holiday-adjusted dates before the 10 October 2023 adoption
of the February 2024 acceleration, plus a pseudo-trigger placebo one quarter before the real
crossing and a descriptive 13G
...
itself and is reported first if it happens. Scope: it is an identity claim, not an estimate of the
February 2024 acceleration, and a referee will ask what the 5 February 2024 filing-window change did.

\paragraph{Candidate B: the slope change after the February 2024 acceleration.}
