...
**Version stamp: 2026-08-21 · post-ledger regeneration · commit `TBD-next`.** An answer written against a stale stamp is
...
**Seven of eight now carry two-pass evidence and move to PROVED.** The protocol (§7) requires an
...
| C1 | On a named region where $L_{\mathcal R}<1$ and $g_r^{PE} > \mathcal B_r^{GE}$, the fixed-policy attenuation sign survives in equilibrium. | CONJECTURE | `threads/thread1_turn1_answer.md`. **Untouched by this regeneration — ticket 29 in flight.** Note that T1's PE-$\Omega$ hypothesis is exactly what fails in GE, so C1's remainder term is the object it has to bound |
