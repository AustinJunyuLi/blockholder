...
\end{aligned}
\]
So $s_B$ is the noise sensitivity of the premium mass the shorter clock removes from the pool, per
unit of pooled mass removed. Under the extra clause of (C-2) it splits into the two parts just named,
\begin{equation}
...
\item \emph{Fixed policies.} The plan menu, the execution policies and the cutoff vector are held
  ...
  flow into the blockholder's executed order path.
\item \emph{Differentiability.} The two conditional expectations in \eqref{eq:caught-sens} are
  ...
  $\tilde s_B$ and $\delta$, also $\Eop[h^{(T)}\mid B]$.
\item \emph{Non-degeneracy.} $\Prb(B) > 0$, $0 < \Omega(\tau,T)$, $\Omega(\tau,T') < 1$, and
  $s_A \ne 0$, equivalently $\mathcal{S}_P(\tau,T) > 0$.
\end{enumerate}
...
\item \emph{The cut identity.}
  ...
  the mass-weighted average of the sensitivity of what the shorter clock leaves in the pool and the
  sensitivity of what it takes out.
...
\item \emph{Readings.} Write $\rho = s_B/s_A$, which is well defined by (C-3).
  ...
  \item If $s_A$ and $s_B$ share a sign, then
  ...
  \item If the cut does not reverse the pooled sensitivity, $s_{A\setminus B}\,s_A \ge 0$, then
    $C_T \le 1$ if and only if $\rho \ge 1$, which under a common sign reads $|s_B| \ge |s_A|$.
    For a common sign the proviso is $|s_B| \le |s_A|/\varphi$.
...
and only if $1-\varphi\rho\le1-\varphi$, that is $\rho\ge1$; since $\varphi>0$, $\rho\ge1$ forces a
common sign, under which $\rho\ge1$ is $|s_B|\ge|s_A|$. Under a common sign the proviso
...
share of the pool faces only the first two requirements. Part~(v) does the same for the criterion
