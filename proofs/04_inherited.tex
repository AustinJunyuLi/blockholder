...
the flagged cell by Steps~9 and~12. Since $\Omega>0$, the elementary definition of conditioning on a positive-probability event gives
\begin{equation}
\label{eq:flagged-MF}
...
When both cells contain a history the map is onto $\{\mathcal C_F, \mathcal C_P\}$; when
$\Omega \in \{0,1\}$ one cell is empty of histories and the map hits only the other, which is
the case Step~7 records. The public flag coordinate is what carries this step: without it
...
applies at each threshold and the noise sensitivity of the expected engagement premium is the pooled share times the pooled sensitivity,
\[
\mathcal S(\kappa,\tau,T) \;=\; \bigl(1-\Omega(\tau,T)\bigr)\,\mathcal S_P(\kappa,\tau,T),
\qquad
\mathcal S(\kappa,\tau',T) \;=\; \bigl(1-\Omega(\tau',T)\bigr)\,\mathcal S_P(\kappa,\tau',T) .
...
$0 < \Omega(\tau,T') < 1$, and $\mathcal S_P(\kappa,\tau,T) > 0$. Assume the hypotheses of
Proposition~\ref{prop:factorisation} hold at both clocks, and that
$\kappa \mapsto M_P(\kappa,\tau,T')$ is differentiable. Write
\begin{equation}
\label{eq:clock-legs}
...
times the pooled sensitivity. Differentiability of $\kappa \mapsto M_P(\kappa,\tau,T')$ makes
$\mathcal S_P(\kappa,\tau,T')$ exist, so $C_T$ in \eqref{eq:clock-legs} is defined. At each clock,
\[
\mathcal S(\kappa,\tau,T) \;=\; \bigl(1-\Omega(\tau,T)\bigr)\,\mathcal S_P(\kappa,\tau,T),
\qquad
\mathcal S(\kappa,\tau,T') \;=\; \bigl(1-\Omega(\tau,T')\bigr)\,\mathcal S_P(\kappa,\tau,T') .
