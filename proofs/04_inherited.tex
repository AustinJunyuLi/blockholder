...
the flagged cell by Steps~9 and~12. Since $\Omega>0$, the partition result gives
\begin{equation}
\label{eq:flagged-MF}
...
sum of two coordinates of a Borel vector and is therefore Borel. By \ref{p:cutoff} the map
$j(\cdot)$ is a step function whose breakpoints are finite in number, by \ref{p:menu}, so it is a
finite sum of indicators of half-lines and hence Borel. The set $\{s : j(s) = j\}$ is Borel for each
$j \in \mathcal J$.
...
states; the claim is about control-node histories. By \ref{p:public} each pooled history carries the
coordinate ``the filing has landed by date $d$'', so the flag is public by construction, and
$\mathcal I_H$ contains that coordinate at $d = H$. By \ref{p:clock} the filing lands exactly at
...
respect to the control-node history and the map from a history to its cell is single-valued and onto
$\{\mathcal C_F, \mathcal C_P\}$. The public flag coordinate is what carries this step: without it
...
whatever differentiability $\partial_\kappa M_P$ has in a policy coordinate. That transfer creates
...
applies at each threshold and the noise sensitivity of the price is the pooled share times the pooled sensitivity,
\[
\mathcal S(\kappa,\tau,T) \;=\; \bigl(1-\Omega(\tau,T)\bigr)\,\mathcal S_P(\kappa,\tau,T),
\qquad
\mathcal S(\kappa,\tau',T) \;=\; \bigl(1-\Omega(\tau',T)\bigr)\,\mathcal S_P(\kappa,\tau',T) .
...
composition leg has to be signed, since $W_\tau \le 1$ leaves $W_\tau C_\tau \le 1$ resting on
$C_\tau$ alone whenever $C_\tau \le 1$.
...
two window margins $T' < T$, so that $T'$ is the shorter clock. Assume $0 < \Omega(\tau,T) < 1$ and
$0 < \Omega(\tau,T') < 1$, and $\mathcal S_P(\kappa,\tau,T) > 0$. Write
\begin{equation}
\label{eq:clock-legs}
...
\emph{The ratio identity.} By the factorisation, at each clock the noise sensitivity of the price is
the pooled share times the pooled sensitivity,
\[
\mathcal S(\kappa,\tau,T) \;=\; \bigl(1-\Omega(\tau,T)\bigr)\,\mathcal S_P(\kappa,\tau,T),
\qquad
\mathcal S(\kappa,\tau,T') \;=\; \bigl(1-\Omega(\tau,T')\bigr)\,\mathcal S_P(\kappa,\tau,T') .
