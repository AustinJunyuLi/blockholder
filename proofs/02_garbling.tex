...
curvature statement about $h$ is read as a statement about $\mathsf h$ on the convex hull of the
pairs $(\pi,\hat v)$ the pooled cell generates. The premium objects are
...
is reclassified. The argument used $b_0 < \tau'$, so that no crossing predates round zero at
either threshold, and it used no property of the kernel.
