...
def fig_cutoff_structure(output_dir: str, data_dir: str) -> None:
    ...
    for _, row in regions.iterrows():
        ...
        # Use white text on dark bands, black on light bands
        dark_bands = {"Public Voice", "Quiet Voice"}
        label_color = "white" if name in dark_bands else "black"
        ...
        if name == "Quiet Voice":
            ...
            ax.text(mid_x, mid_y, name, ha="center", va="center", fontsize=9, fontweight="bold", color=label_color)

    # Cutoff markers: short vertical lines spanning the band only, labels below.
    for x, label in [(k1, r"$k_1$"), (k0, r"$k_0$"), (kD, r"$k_D$")]:
        ax.plot([x, x], [y0 - 0.08, y0 + h + 0.08], linestyle="--", linewidth=1.0, color="#993BFF", zorder=2)
...
def fig_prices(output_dir: str, data_dir: str) -> None:
    ...
    for ax, D in zip(axes, [0, 1]):
        ...
        ax.scatter(sub["X"].astype(str), sub["P_trade"], marker="D", s=36, zorder=3, color="#D4A843", edgecolors="#8B6914", linewidths=0.6, label=r"$P_{trade}(X)$")
...
def fig_timeline(output_dir: str, data_dir: str) -> None:
    ...
    # Color-coded annotations (dark=choices, mid=observables, bright=outcomes)
    blue = "#361A54"    # choices (UCL dark purple)
    rose = "#6B3FA0"    # observables (UCL mid purple)
    green = "#993BFF"   # outcomes (UCL bright purple)
    ...
    # t=1.5: Bidder arrives, sees the public signal, draws xi
    ax.text(3, -0.15, "Bidder arrives w.p. $\\lambda_B$",
            ha="center", va="top", fontsize=8.5, color="black")
    ax.text(3, -0.28, "Sees public signal, draws $\\xi$",
            ha="center", va="top", fontsize=8.5, color=rose)
