...
def equilibrium_residual(k1: float, k0: float, kD: float, params: ModelParams) -> float:
    ...
    def U_engage(s: float) -> float:
        # Engagement actions: Quiet Voice (no disclosure) or Public Voice (disclosed).
        return max(U(Action.QUIET, s), U(Action.PUBLIC, s))

    s_min = params.mu - SIGMA_GRID * params.sigma_s
    s_max = params.mu + SIGMA_GRID * params.sigma_s

    # k1 condition: EXIT vs best of {HOLD, QUIET, PUBLIC}
    diff1 = U(Action.EXIT, k1) - max(U(Action.HOLD, k1), U(Action.QUIET, k1), U(Action.PUBLIC, k1))
    ...
    # k0 condition: HOLD vs best engagement action (if region exists)
    if (k0 - k1) > TOL_REGION:
        r0 = abs(U(Action.HOLD, k0) - U_engage(k0))
    else:
        # hold region collapsed: require engagement weakly dominates HOLD just above k1
        s_test = min(k1 + TOL_REGION, s_max)
        r0 = max(0.0, U(Action.HOLD, s_test) - U_engage(s_test))
...
def solve_equilibrium(
    ...
    for _it in range(max_iter):
        ...
        # ------------------------------------------------------------------
        # 1) k1: EXIT vs best of {HOLD, QUIET, PUBLIC}
        # ------------------------------------------------------------------
        def diff_exit_best(s: float) -> float:
            return U(Action.EXIT, s) - max(U(Action.HOLD, s), U(Action.QUIET, s), U(Action.PUBLIC, s))
        ...
        # ------------------------------------------------------------------
        # 2) k0: HOLD vs best engagement action above k1
        # ------------------------------------------------------------------
        def diff_hold_engage(s: float) -> float:
            return U(Action.HOLD, s) - max(U(Action.QUIET, s), U(Action.PUBLIC, s))

        # search on [k1_new, s_max]
        s0 = min(k1_new + 1e-4, s_max)
        br = _bracket_root(diff_hold_engage, s_grid, low=k1_new, high=s_max)
        if br is None:
            # if engagement dominates immediately, collapse HOLD region
            k0_new = k1_new if diff_hold_engage(s0) <= 0.0 else s_max
        else:
            a, b = br
            k0_new = a if a == b else float(brentq(diff_hold_engage, a, b))
