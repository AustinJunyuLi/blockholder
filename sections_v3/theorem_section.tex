...
%  commit <pending-orchestrator-hash>), section 6
...
Definition~\ref{def:theta}. \emph{Card note, 2026-08-30:} the card row expands this reading in
its bracket half: there is a fixed compact signal interval --- \textbf{the common bracket}
$[s_{\mathrm{lo}},s_{\mathrm{hi}}]$ at the given $\vartheta$, common in $k \in \Theta$ and in the
adjacent plan pair, of which $\Theta$ is the ordered part
($s_{\mathrm{lo}} \le k_1 \le \dots \le k_{J-1} \le s_{\mathrm{hi}}$; the construction recorded
for Steps 13--14 in the card's A6 note) --- such that for every $k \in \Theta$ and every adjacent
plan pair the best-response indifference signal \textbf{exists} and lies in the bracket, which is
what makes the finite cutoff vector represent the best-response plan map on the whole Gaussian
signal line and not merely inside the bracket. The clause excludes every menu-and-price-system at
which some adjacent indifference signal fails to exist --- never-selected middle plans as well as
the dominated extreme plans whose positive-probability Gaussian-tail failure motivated it
(polish-pass F5); the stronger form is taken so the cutoff vector carries interior data at every
$k$, and the exclusion is a named domain narrowing
(\texttt{research/model\_v4/proofs/P1\_proof.md} Steps 13--14; polish-pass audit 2026-08-29,
P1-R40-A; ruled Austin's route choice 2026-08-29; bracket defined and narrowing recorded by the
2026-08-30 re-derivation, change 1 --- card P1 row).
...
\item \emph{Whether the continuity clause of Assumption~\ref{asm:A6} holds for the declared
  ...
  claimed nor shown anywhere. \emph{Card note, 2026-08-29:} clause (a)'s ``$t$-constrained
  Kakutani route already on file'' is superseded on the card:
  \texttt{research/model\_v4/proofs/P1\_proof.md} Step 18 as it stands is a scoped remark ---
  its Kakutani conclusion is withdrawn as unestablished, and it carries no $t$-constrained game
  and no $t \downarrow 0$ limit --- so that route is a sketch, not a repair on file (A6 record,
  2026-08-29 pointer correction).
\end{enumerate}
