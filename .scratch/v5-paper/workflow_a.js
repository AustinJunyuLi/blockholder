...
async function twoPass(label, writePrompt, rederivePrompt, E) {
  E = Object.assign({ write: "xhigh", rederive: "high", fix: "xhigh", rederive2: "high" }, E || {})
  let w = await agent(`${PRE}\n\n${writePrompt}`, { model: "opus", effort: E.write, schema: RESULT, label: `${label} write` })
...
const chains = [
  async () => {
    results.t01 = await attempt('01', 'Ticket 01-mark-parameter-and-timed-smoke.md. Build exactly what the ticket says. Report wall time and peak memory for one node at mark=2, H=10, and the diff at mark=1 against numerical_v4/smoke_output.txt.', { phase: 'Theory code', effort: 'high', retryEffort: 'xhigh' })
    if (results.t01.status !== 'PASS') return
    results.t01v = await agent(`${PRE}\n\nTicket 01, verifier. You did not write the change. Rerun .venv/bin/python -m numerical_v4.smoke and one check script at mark=2 and confirm the numbers in this report match: ${JSON.stringify(results.t01)}. Return PASS or FAIL with reasons.`, { model: 'opus', effort: 'low', schema: VERDICT, label: '01 verify', phase: 'Theory code' })
  },
  async () => {
  ...
  async () => {
    ...
    results.t04 = await pipeline(four,
      ...
      async (v, stmt) => {
        ...
        const w = await agent(`${PRE}\n\nTicket 04. A re-deriver failed on ${stmt} with reasons: ${v ? v.reasons : 'none'}. Repair the proof once, working from the inherited proof in inherited/draft_v3/ (never cited), and write it into proofs/04_inherited.tex.`, { model: 'opus', effort: 'xhigh', schema: RESULT, label: '04 repair', phase: 'Proofs' })
      ...
      async (r, stmt) => {
        ...
        await agent(`${PRE}\n\nTicket 04. Transcribe the proof of ${stmt} into proofs/04_inherited.tex in the paper's notation, from the inherited proof (never cited), and report the file changed.`, { model: 'opus', effort: 'medium', schema: RESULT, label: '04 transcribe', phase: 'Proofs' })
        return r
      })
  },
  async () => {
    results.t06 = await attempt('06', 'Ticket 06-empirics-build-and-e1-run.md. Build empirics/fingerprints.py and empirics/test_fingerprints.py to empirics/spec.md exactly, run build and run e1, and report the coverage counts and gate values.', { phase: 'Empirics', effort: 'high', retryEffort: 'xhigh' })
    if (results.t06.status !== 'PASS') return
    results.t06v = await agent(`${PRE}\n\nTicket 06, second agent. Run the tests and rerun e1 from the cache; confirm e1_estimate.json is byte-identical to the file the builder produced (compare against this report: ${JSON.stringify(results.t06)}). PASS or FAIL.`, { model: 'opus', effort: 'low', schema: VERDICT, label: '06 rerun', phase: 'Empirics' })
    results.t07 = await attempt('07', 'Ticket 07-e1-blind-audit.md. You did not write the parser or the loader. Do the sixty-case audit and write gate E1-G2 into e1_estimate.json.', { phase: 'Empirics', effort: 'medium' })
  },
  async () => {
    results.t09 = await attempt('09', 'Ticket 09-literature-check.md. Answer the three questions with primary sources and write docs/lit_check_2026-09.md.', { phase: 'Literature', agentType: 'general-purpose', effort: 'medium' })
  },
]
...
if (results.t01 && results.t01.status === 'PASS') {
  ...
  if (results.t02 && results.t02.status === 'PASS' && results.t02.write && results.t02.write.named_condition) {
    gridJobs.push(() => attempt('02 grid', `Ticket 02, grid verification. Write numerical_v4/checks/t5_threshold_condition.py that checks this named condition at every calibration node at mark=2 and writes a JSON record: ${results.t02.write.named_condition}. Report the node count and the verdict per node.`, { phase: 'Grid checks', effort: 'medium', retryEffort: 'high' }))
  }
  if (results.t03 && results.t03.status === 'PASS') {
    gridJobs.push(() => attempt('03 grid', 'Ticket 03-who-gets-caught.md, grid check section. Write numerical_v4/checks/t5_who_gets_caught.py exactly as the ticket describes and report the comparison table against the T1 record.', { phase: 'Grid checks', effort: 'medium', retryEffort: 'high' }))
  }
