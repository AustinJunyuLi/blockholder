...
let w = await attempt('11', 'Ticket 11-paper-writer.md. Write paper.tex, appendix.tex and paper.bib as the ticket says. Apply the unslop rules to the prose.', { phase: 'Write', effort: 'xhigh', retryEffort: 'max' })
let c = null
if (w.status === 'PASS') {
  c = await agent(`${PRE}\n\n${CHECK}`, { model: 'opus', effort: 'low', schema: VERDICT, label: '11 check', phase: 'Check' })
  if (!(c && c.verdict === 'PASS')) {
    ...
    c = await agent(`${PRE}\n\n${CHECK}`, { model: 'opus', effort: 'low', schema: VERDICT, label: '11 check 2', phase: 'Check' })
  }
}
