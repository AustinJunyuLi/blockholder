...
const ref = await agent(`${PRE}\n\nTicket 12-referee.md. You wrote nothing in this session. Referee deliverable-quality: read paper.pdf and appendix.pdf (render pages to images if needed), write .scratch/v5-paper/referee_report.md with blocking and minor lists, each with a location. Do not edit the paper.`, { model: 'opus', effort: 'high', schema: RESULT, label: '12 referee', phase: 'Referee' })
...
if (fix && fix.status === 'PASS') {
  deliver = await attempt('14', 'Ticket 14-compile-and-deliver.md. Compile, inspect every page, apply the unslop gate, copy the PDFs to deliverable/.', { phase: 'Deliver', effort: 'medium', retryEffort: 'high' })
}
