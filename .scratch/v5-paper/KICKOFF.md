...
prompt: `.scratch/v5-paper/grok/README.md`.
...
> Austin says a batch is done, read runs/batch-N/result.txt and the diff, run the Opus attacks
> and judge calls the README lists for that checkpoint, start the check runs detached one at a
> time (ADR 0004), commit one concern per commit, write grok/checkpoint-N.md and finalise
> grok/batch-(N+1).md. A STOP ends the build: write the one-page judgment and wait for Austin.
> You own git; no worker runs any.
