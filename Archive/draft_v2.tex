...
\begin{proof}
By Lemma~\ref{lem:endpoints}, the continuous extension of $\Delta^{\min}$ to the closed interval $[0,1]$ satisfies $\Delta^{\min}(\kappa)\to m_0\PP(\textup{bid})$ as $\kappa \uparrow 1$. Because the normal probability formulas, Bayesian posteriors, and pricing mapping are continuous in $\kappa$, the function $\Delta^{\min}(\kappa)$ is continuous on the compact set $[0,1]$. By the Weierstrass extreme value theorem, it attains a global maximum.
